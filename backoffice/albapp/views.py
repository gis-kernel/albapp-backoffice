    ...
    try:
        ...
        for batch in batches:
            ...
            if request.method == "POST" and element_format:
                ...
                if shp_metadata:
                    centroid = shp_metadata.gps_coords
                    
                    zipcode = shp_metadata.zipcode
                    fire_situation["unique_reference"].append({"gps_coords": centroid})
                    if zipcode:
...
def update_fire_situation(request, batch_id):
    ...
    centroid = shp_metadata.gps_coords if shp_metadata else None
    zipcode = shp_metadata.zipcode if centroid else None
    ...
    return render(request, "batch_detail.html", {
        ...
        "gps_coords": centroid if centroid else [],
...
def update_batch_details(request, batch):
    ...
    gps_coords = request.POST.get("gps_coords")
    if status:
        batch.status = status
    if tags:
        batch.tags = tags
    ...
    if gps_coords:        
        gps_coords_list = ast.literal_eval(gps_coords)
        # print(gps_coords_list)
        Shp_metadata = ShpMetadata.objects.filter(batch = batch).first()
        if Shp_metadata:
            Shp_metadata.gps_coords = gps_coords_list           
            zipcode,place = get_zipcode_from_coordinates(gps_coords_list[0], gps_coords_list[1])
            Shp_metadata.zipcode = zipcode
            Shp_metadata.place_name = place
            Shp_metadata.save() 
        ...
        if "results" in data and len(data["results"]) > 0:
            for component in data["results"][0]["components"]:
                if "postcode" in data["results"][0]["components"]:
                    ...
                    if "municipality" in data["results"][0]["components"]:
                        municipality = data["results"][0]["components"]["municipality"]
                        print(postcode,municipality)
                        return postcode,municipality
                    else:
...
def delete_all(request):
    if request.method == 'POST':
        bucket_name = settings.AWS_STORAGE_BUCKET_NAME
        folder_prefixes = ['videos', 'shapefiles', 'images']
        list_and_delete_objects(bucket_name, folder_prefixes)
        
        tables_to_delete = ['shp_metadata', 'image_metadata', 'video_metadata', 'batch']
        delete_tables(tables_to_delete)        
        return redirect('index', status=200)
    else:
...
def list_and_delete_objects(bucket_name, folder_prefixes):
    try:
        for prefix in folder_prefixes:
            response = s3.list_objects_v2(Bucket=bucket_name, Prefix=prefix)
            if 'Contents' in response:
                print(f'Objects in folder {prefix} in bucket {bucket_name}:')
                for obj in response['Contents']:
                    if obj["Key"] != prefix and obj["Key"] != f'{prefix}/':
                        file_name = obj["Key"]
                        s3.delete_object(Bucket=bucket_name, Key=file_name)
                        print(f'Deleted {file_name} from {bucket_name}')
            else:
                print(f'No objects found in folder {prefix} in bucket {bucket_name}')
...
#             centroid = shp_metadata.gps_coords
#             response_data = {
#                 "gps_coordinates": [centroid[0], centroid[1]],
